...
void svgRectArea(FILE *svg, float x, float y, float w, float h) {

    fprintf(svg, "\n\t<rect x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" fill=\"%s\" stroke=\"%s\"/>", x, y, w, h, "transparent", "red");
}
...
void svgAnchor(FILE *svg, float x, float y) {
    fprintf(svg, "\n\t<circle cx=\"%f\" cy=\"%f\" r=\"%f\" fill=\"%s\" stroke=\"%s\"/>", x, y, 2.0, "red", "red");
}
